...
ui<- shinyUI(fluidPage(
  ...
  navbarPage(
    ...
    tabPanel("STA 631 Final Portfolio Extension", 
             titlePanel(
               div(
                 class = "main-panel",
                 br(),
                 h3("STA 631 Final Portfolio")
               )
             ),
             tabsetPanel(
               id = "objectives-tabs",  # Unique id for the tabsetPanel
               type = "tabs",   # Type of tabsetPanel (tabs)
               tabPanel("Objective 1", "Describe probability as a foundation of statistical modeling, including inference and maximum likelihood estimation"),
               tabPanel("Objective 2", "Determine and apply the appropriate generalized linear model for a specific data context"),
               tabPanel("Objective 3", "Conduct model selection for a set of candidate models"),
               tabPanel("Objective 4", "Communicate the results of statistical models to a general audience"),
               tabPanel("Objective 5", "Use programming software (i.e., R) to fit and assess statistical models")
             )
    ),
...
server<- function(input, output, session) {
  ...
  observe({
    shinyjs::toggle(id = "dropdown-menu", condition = input$dropdown %in% 1:5)
  })
