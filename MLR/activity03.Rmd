...
```{r}
hfi = readr::read_tsv("https://www.openintro.org/data/tab-delimited/hfi.txt")
...
1. For each pair of variables, how would you describe the relationship graphically?
  ...
  There's a positive relationship between pf score and pf control. Additionally, there is a negative relationship between pf rank and pf score. As one's score increases, their rank decreases. Furthermore, the expression control has a negative relation with rank. As exprresson control increases, the rank decreases.
...
3. Are your two explanatory variables collinear (correlated)?
  ...
  Yes- they seem highly correlated with one another in a negative way. 
...
$$
\hat{y} = 9.3 + .02 \times x_1 + -.03 \times x_2
$$
...
5. For each of the estimated parameters (the *y*-intercept and the slopes associated with each explanatory variable - three total), interpret these values in the context of this problem.
  ...
  If the expression control and rank were equal to zero, the personal freedom score would be equal to 9.3. For every unit increase of expression control while Rank is held CONSTANT, the personal freedom score increases by .02. Similarly, for every unit increase of rank while expression control is held CONSTANT, the pf score decreases by .03 units. 
...
I think I like looking at the GGpairs output more than 3d visual. 3D can be difficult to interpret. Additionally Git has difficulty loading 3d images. 
...
Based on the table, it looks like the binary indicator was assigned YES if the region is NOT Latin America and The Caribbean. 
...
It looks like the west_atlantic is only looking at "Yes".. I'm not sure if that impacts things? 
...
It looks like the Latin America regions would be missing? 
...
y = 4.377 + -.1 \times \text{west_atlanticYES} + .54 \times \text{pf_expression_control} + \varepsilon
...
qual_mod <- lm_spec %>% 
  fit(pf_score ~ west_atlantic + pf_expression_control, data = hfi_2016_update)

# model output
tidy(qual_mod)

# By including char drawing sep lines for each of the categories. When not west of atlantic .1 higher than when it IS west of the atlantic. (Keeps it constant)
...
```
It looks like every region is included into the model which is interesting.. I think it would b a bit difficult to interpret. 
